...
export const createFormulaire = (formulaireData) => (dispatch) => {
  ...
  fetch('http://localhost:8080/formulaire/new', {
    method: 'POST',
...
export const getAllFormulaires = () => (dispatch) => {
    ...
    fetch('http://localhost:8080/formulaire/forms')
        .then((response) => response.json())
        .then((data) => {
            dispatch({
...
export const getFormulaire = (id) => (dispatch) => {
  ...
  fetch(`http://localhost:8080/formulaire/formulaires/${id}`, {
      method: 'GET',
  })
  .then((response) => {
      if (!response.ok) {
...
export const updateFormulaire = (id, formulaireData) => (dispatch) => {
  ...
  fetch(`http://localhost:8080/formulaire/forms/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(formulaireData),
  })
